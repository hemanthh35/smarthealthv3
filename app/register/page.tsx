...
import { Eye, EyeOff, Lock, Mail, Heart, Sparkles, ArrowRight, Shield, Zap, User, CheckCircle } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  const passwordStrength = () => {
    ...
    if (password.length === 0) return { strength: 0, color: 'bg-gray-300' }
    if (password.length < 6) return { strength: 1, color: 'bg-red-500' }
    if (password.length < 8) return { strength: 2, color: 'bg-yellow-500' }
    if (password.length < 10) return { strength: 3, color: 'bg-blue-500' }
        ...
        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="bg-white/10 backdrop-blur-md rounded-3xl p-8 md:p-12 border border-white/20 shadow-2xl max-w-md w-full">
            ...
            <div className="text-center mb-8">
              <div className="flex items-center justify-center w-16 h-16 bg-gradient-to-r from-green-500 to-blue-600 rounded-2xl mx-auto mb-4 shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {[1, 2, 3, 4].map((level) => (
                      <div
                        ...
                        className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                          level <= strength ? color : 'bg-white/20'
                        }`}
                      />
                    ))}
                  </div>
                  <p className="text-xs text-blue-200 mt-1">
                    Password strength: {strength === 0 ? 'None' : strength === 1 ? 'Weak' : strength === 2 ? 'Fair' : strength === 3 ? 'Good' : 'Strong'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start">
                <input
                  ...
                  onChange={handleInputChange}
                  required
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-white/20 rounded bg-white/10 mt-1"
                />
                <label htmlFor="agreeToTerms" className="ml-2 text-sm text-blue-200">
                  I agree to the{' '}
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-green-500 to-blue-600 text-white py-3 px-6 rounded-xl font-semibold hover:shadow-lg hover:shadow-green-500/25 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
              >
                {isLoading ? (
                  <>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white/5 backdrop-blur-md rounded-xl p-4 border border-white/10 text-center">
              ...
              <p className="text-xs text-blue-200">No hidden costs</p>
            </div>
            <div className="bg-white/5 backdrop-blur-md rounded-xl p-4 border border-white/10 text-center">
              <Shield className="w-6 h-6 text-blue-400 mx-auto mb-2" />
              <h3 className="text-sm font-semibold text-white mb-1">Secure</h3>
              <p className="text-xs text-blue-200">Privacy first</p>
            </div>
            <div className="bg-white/5 backdrop-blur-md rounded-xl p-4 border border-white/10 text-center">
